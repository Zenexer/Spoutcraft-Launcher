...
public class SettingsUtil {
	...
	private static void setProperty(String s, Object value) {
		if (settings.checkProperty(s)) settings.changeProperty(s, value);
		else settings.put(s, value);
	}
	...
	private static boolean isProperty(String s, boolean def) {
		if (settings.checkProperty(s)) { return settings.getPropertyBoolean(s); }
		settings.put(s, def);
		return def;
	}

	private static int getProperty(String s, int def) {
		if (settings.checkProperty(s)) { return settings.getPropertyInteger(s); }
		settings.put(s, def);
		return def;
	}

	private static String getProperty(String s, String def) {
		if (settings.checkProperty(s)) { return settings.getPropertyString(s); }
		settings.put(s, def);
		return def;
	}
